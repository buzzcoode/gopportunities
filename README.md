...
1. Clone the repository: `git clone https://github.com/buzzcoode/weworkanywhere.git`
...
- `make build`: Build the application and create an executable file named `weworkanywhere`.
- `make test`: Run tests for all packages in the project.
- `make docs`: Generate the API documentation using Swag.
- `make clean`: Remove the `weworkanywhere` executable and delete the `./docs` directory.
...
- `docker build -t weworkanywhere-api .`: Build a Docker image for the project. Replace `weworkanywhere-api` with a name for your image.
- `docker run -p 8080:8080 -e PORT=8080 weworkanywhere-api`: Run a container based on the built image. Replace `your-image-name` with the name you used when building the image. You can change the port number if necessary.
